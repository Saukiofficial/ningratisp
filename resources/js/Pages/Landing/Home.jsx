...
export default function Home({ packages }) {
    ...
    const handleCheckCoverage = (e) => {
        e.preventDefault();
        post(route('landing.check-coverage'), {
            preserveScroll: true,
            onSuccess: () => {
    ...
    return (
        <div className="font-sans antialiased text-gray-900 bg-white relative">
            ...
            {showAlert && (flash?.success || flash?.error) && (
                <div className={`fixed top-24 left-1/2 transform -translate-x-1/2 z-50 w-11/12 max-w-lg p-4 rounded-xl shadow-2xl flex items-start gap-4 border-l-4 animate-fade-in-down transition-all duration-500 ease-in-out ${
                    flash.success
